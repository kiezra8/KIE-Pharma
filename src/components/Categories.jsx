...
export default function Categories({ isPage, onToggle }) {
  ...
  if (loading) return <div style={{padding: '40px', textAlign: 'center'}}>Loading catalog...</div>;
        ...
        {classProducts.length > 0 ? (
          ...
          <div>
            ...
            <div className="empty-category-state fade-in">
              ...
              <h4>Coming Soon</h4>
              <p>We are currently stocking up on {selectedSub.name}.</p>
            </div>
          </div>
        )}
      </div>
  ...
  if (activeCategory) {
    const subcats = subcategories.filter(s => s.category_id === activeCategory.id);
      ...
      <div className="categories-page-wrapper">
        ...
        {subcats.length > 0 ? (
            ...
            {subcats.map(sub => (
              <div key={sub.id} className="subcategory-card-premium" onClick={() => setSelectedSub(sub)}>
                <div className="sub-card-img">
                  <img src={sub.img || sub.image} alt={sub.name} />
                </div>
          ...
          <div className="empty-category-state fade-in">
            ...
            <p>Direct view enabled for this category.</p>
          </div>
        )}
      </div>
    );
